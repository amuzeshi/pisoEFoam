...
namespace Foam
{
    defineTypeNameAndDebug(NernstPlanck_steady, 0);
    addToRunTimeSelectionTable(EDFEquation, NernstPlanck_steady, dictionary);
}

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::NernstPlanck_steady::NPSpecie::NPSpecie
(
    const word& name,
...
Foam::NernstPlanck_steady::NernstPlanck_steady
(
    const word& name,
    const surfaceScalarField& phi,
    const dictionary& dict
...
Foam::tmp<Foam::volVectorField> Foam::NernstPlanck_steady::Fe() const
{    
...
void Foam::NernstPlanck_steady::correct()
{
